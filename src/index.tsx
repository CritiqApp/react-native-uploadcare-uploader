...
// Define the emitter
const emitter = Platform.OS === 'ios' ? new NativeEventEmitter(NativeModules.UploadcareUploader) : DeviceEventEmitter
...
const callbackRegistry: {[id: number]: {
...
emitter.addListener('upload_session_progress', (data: { session_id: string, current: number, total: number }) => {
...
// Process an upload progress
emitter.addListener('media_uuid_created', (data: { session_id: string, uuid: string}) => {
  const { session_id, uuid } = data
  let onUUIDCreated = callbackRegistry[sessionToId[session_id]]?.onUUIDCreated
  onUUIDCreated && onUUIDCreated(uuid)
})

function upload(key: string, file: {
  uri: string
  mimeType: string
}, params: {
